...
from ..schemas.exam_type import ExamCreate


async def get_exams(db: Session, skip: int = 0, limit: int = 200):
    db_exams = db.query(Exam_Type).offset(skip).limit(limit).all()
    return db_exams
...
async def create_exam(db: Session, exam: ExamCreate, creation_user: str):
    db_exam = Exam_Type(name=exam.name.upper(), gravity=exam.gravity, creation_user= creation_user)
    db.add(db_exam)
    db.commit()
    db.refresh(db_exam)
    return db_exam
