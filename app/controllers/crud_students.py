...
async def delete_student_by_id(db: Session, id: int):
    db_student = Student.query.filter_by(id=id).one()
...
async def get_student_sections(db: Session, id: int):
    return db.query(association_table_students_sections).filter(association_table_students_sections.student_id == id).all()