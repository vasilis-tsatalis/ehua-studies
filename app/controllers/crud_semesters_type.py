...
async def create_semester(db: Session, semester: SemesterCreate, creation_user: str):
    db_semester = Semester_Type(name=semester.name.upper(), description=semester.description, start_date=semester.start_date, end_date=semester.end_date, creation_user= creation_user)
    db.add(db_semester)
    db.commit()
